...
MC_LINUX_BIN="$MC_ROOT/runtime/linux-bin"
MC_LINUX_WSL_SHIM_SOURCE="$MC_ROOT/tools/linux/wsl.exe"
if [ ! -f "$MC_LINUX_WSL_SHIM_SOURCE" ]; then
  echo "Missing Linux wsl.exe shim source: $MC_LINUX_WSL_SHIM_SOURCE" >&2
  exit 1
fi
mkdir -p "$MC_LINUX_BIN"
cp "$MC_LINUX_WSL_SHIM_SOURCE" "$MC_LINUX_BIN/wsl.exe"
chmod 0755 "$MC_LINUX_BIN/wsl.exe"
export PATH="$MC_LINUX_BIN:$PATH"
export MAIN_COMPUTER_EXECUTOR_WSL_COMMAND="${MAIN_COMPUTER_EXECUTOR_WSL_COMMAND:-wsl.exe}"
...
if normalized != data:
    ...
    print(f"Normalized CRLF line endings for Linux execution: {path}", file=sys.stderr)
PY
}

normalize_lf_script "$MC_ROOT/docker/executor/main-computer-exec"
chmod 0755 "$MC_ROOT/docker/executor/main-computer-exec" 2>/dev/null || true
