TASK: Generate one experimental JavaScript gameplay pack for Main Computer from the approved plan.
...
ONLY AVAILABLE ENCOUNTER METHODS:
[
  "onStart",
  "onHostilesDefeated",
  "onAllHostilesDefeated",
...
API RULES:
- Use only the methods listed in ONLY AVAILABLE ENCOUNTER METHODS on the encounter object.
- Do not call encounter.getMetadata(...), encounter.getState(...), encounter.query(...), or any engine-looking method not listed above.
- If the plan needs a random choice, define a local constant array in pack.js and choose from it with Math.random(); do not ask the encounter object for metadata.
