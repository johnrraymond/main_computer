#!/usr/bin/env python3
"""Standalone retired activation guardian shim cleanup.

This tool intentionally handles one thing only: installing or verifying the
known inert ``mother-add-node-validator-activation-guardian`` retired health shim.  It does not
clean admission voters, genesis helpers, replica helpers, Docker host
...
KIND = "main_computer.mother.activation_guardian_cleanup.v1"
EVIDENCE_SUBDIR = "mother-activation-guardian-cleanup"

IDENTIFIER_RE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]*$")
UUID_RE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]{2,127}$")

SHIM_NAME = "mother-add-node-validator-activation-guardian"
SHIM_IMAGE = "alpine:3.20"
SHIM_LABEL = "main_computer.mother.post_work_shim"


class MotherActivationGuardianCleanupError(RuntimeError):
    """Activation guardian cleanup failed before a trustworthy result."""
...
def _stderr_progress(quiet: bool = False) -> ProgressCallback | None:
    ...
    def emit(phase: str, message: str, details: Mapping[str, Any]) -> None:
        ...
        print(f"{_utc_now()} mother-activation-guardian-cleanup.{phase} {message}{suffix}", file=sys.stderr)
...
def _operation(network: str, suffix: str) -> OperationIdentity:
    ...
    return OperationIdentity(
        operation_id=f"mother-activation-guardian-cleanup-{suffix}-{stamp}",
        request_id=f"mother-activation-guardian-cleanup-{suffix}-{stamp}-request",
        network=network,
...
def _is_shim_record(item: Mapping[str, Any]) -> bool:
    ...
    # real activation guardian uses python:3.12-alpine; the retired shim is the
...
def _shim_service(service_uuid: str) -> dict[str, Any]:
    return {
        ...
        "command": [
            ...
            (
                "printf '%s\\n' "
                "'retired Mother activation guardian shim; no proof contract is served here' "
...
def _parse_compose(compose_text: str) -> dict[str, Any]:
    try:
        ...
        raise MotherActivationGuardianCleanupError(
            "MOTHER_ACTIVATION_GUARDIAN_CLEANUP_COMPOSE_INVALID",
...
def _validate_compose_or_raise(compose: Mapping[str, Any], *, node: str) -> None:
    ...
    if node not in services:
        raise MotherActivationGuardianCleanupError(
            "MOTHER_ACTIVATION_GUARDIAN_CLEANUP_MAIN_SERVICE_MISSING",
            f"rewritten compose does not contain the target node service {node!r}",
        )
    if SHIM_NAME not in services:
        raise MotherActivationGuardianCleanupError(
            "MOTHER_ACTIVATION_GUARDIAN_CLEANUP_SHIM_MISSING",
            "rewritten compose does not contain mother-add-node-validator-activation-guardian",
        )
    if not _is_shim_definition(SHIM_NAME, services[SHIM_NAME]):
        raise MotherActivationGuardianCleanupError(
            "MOTHER_ACTIVATION_GUARDIAN_CLEANUP_SHIM_INVALID",
            "mother-add-node-validator-activation-guardian is not the retired shim definition",
        )
    ...
    if missing:
        raise MotherActivationGuardianCleanupError(
            "MOTHER_ACTIVATION_GUARDIAN_CLEANUP_COMPOSE_DANGLING_DEPENDS_ON",
...
def _install_shim_in_compose(
    ...
    *,
    ...
    if before_other_services != after_other_services:
        raise MotherActivationGuardianCleanupError(
            "MOTHER_ACTIVATION_GUARDIAN_CLEANUP_SCOPE_VIOLATION",
            "activation guardian cleanup changed non-target services",
        )
...
def inspect_activation_guardian_cleanup(
...
def execute_activation_guardian_cleanup(
    ...
    *,
    ...
    allow_retired_activation_guardian_shim: bool,
    ...
    if _uuid(acknowledged_service_uuid, "acknowledged_service_uuid") != service:
        raise MotherActivationGuardianCleanupError(
            "MOTHER_ACTIVATION_GUARDIAN_CLEANUP_ACK_REQUIRED",
            "acknowledged service UUID must match the target service UUID",
        )
    if not allow_retired_activation_guardian_shim:
        raise MotherActivationGuardianCleanupError(
    ...
    result = {
        ...
        "summary": {
            ...
            "retired_activation_guardian_shim_replaced_existing": bool(
                patch_receipt and patch_receipt.get("retired_activation_guardian_shim_replaced_existing")
            ),
            "retired_activation_guardian_shim_serves_proof": False,
            "retired_activation_guardian_shim_mounts_proof_volume": False,
            "only_target_supported": SHIM_NAME,
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Install or verify the retired mother-add-node-validator-activation-guardian health shim only."
    )
    ...
    parser.add_argument("--allow-retired-activation-guardian-shim", action="store_true")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.mode == "inspect":
            ...
            result = execute_activation_guardian_cleanup(
                ...
                allow_retired_activation_guardian_shim=args.allow_retired_activation_guardian_shim,
    ...
    except (MotherActivationGuardianCleanupError, MotherDeploymentCompletedHelperCleanupError) as exc:
        code = getattr(exc, "code", "MOTHER_ACTIVATION_GUARDIAN_CLEANUP_FAILED")
        print(
            json.dumps(
                {
                    "kind": KIND,
