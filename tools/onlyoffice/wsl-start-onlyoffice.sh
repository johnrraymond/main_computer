...
echo "Starting ONLYOFFICE Docs native services in WSL on port ${PORT}..."

start_service() {
  local service_name="$1"
  if command -v systemctl >/dev/null 2>&1 && systemctl list-unit-files "$service_name.service" >/dev/null 2>&1; then
    mc_sudo systemctl start "$service_name" >/dev/null 2>&1 || true
  fi
  if command -v service >/dev/null 2>&1; then
    mc_sudo service "$service_name" start >/dev/null 2>&1 || true
  fi
}
...
if command -v supervisorctl >/dev/null 2>&1; then
  mc_sudo supervisorctl start all >/dev/null 2>&1 || true
fi
