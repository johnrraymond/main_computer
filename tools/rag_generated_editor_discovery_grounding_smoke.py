...
def discovery_schema() -> dict[str, Any]:
    return {
        ...
        "request_scope": {
            ...
            "authorized_edit_region": "smallest existing literal/source region that must change",
...
Request-scope rules:
- First classify the user task by operation_class.  For common localized tasks
  such as copy/text/content/label/style/behavior changes, the authorized edit
  region is the smallest existing literal string, text node, element, rule, or
  symbol that must change.
- Do not select a whole container, whole page, whole file, or broad section as
  the edit_target when a narrower literal target is visible.
- Do not broaden the task into cleanup, deletion, reordering, redesign, or
  refactoring unless the task explicitly asks for that work.
- If you cannot identify a narrow authorized edit_target from the supplied
  candidate excerpts, set proceed=false instead of guessing.
...
For localized tasks, include preservation anchors for adjacent sibling content,
nearby metadata/list rows, imports/assets/scripts/styles, container boundaries,
or any nearby source that is not explicitly requested to change.
