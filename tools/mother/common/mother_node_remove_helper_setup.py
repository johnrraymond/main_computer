...
def _escape_compose_interpolation(value: str) -> str:
    """Preserve shell variables through Docker Compose interpolation.

    Compose expands $VAR and ${VAR} while loading the service definition.  The
    runner script intentionally contains shell variables that must be expanded
    inside the runner container, not by Compose/Coolify while materializing the
    service.
    """
    return value.replace("$", "$$")
...
def _runner_compose(service_name: str, runner_script: str) -> str:
    compose = {
        "services": {
            service_name: {
                "image": "docker:27-cli",
                "command": ["sh", "-lc", _escape_compose_interpolation(runner_script)],
