...
def _bootstrap_v2_proof_compose(compose: str) -> str:
    marker = "  mother-genesis-proof-guardian:"
    volumes_marker = "\nvolumes:\n"
    if marker not in compose or volumes_marker not in compose:
        raise _fail(
            "MOTHER_DEPLOY_NODE_ADD_SINGLE_NODE_BOOTSTRAP_GUARDIAN_COMPOSE_INVALID",
            "single-node bootstrap proof Compose is missing the guardian or volumes section",
        )
    before, guardian_tail = compose.split(marker, 1)
    guardian, after = guardian_tail.split(volumes_marker, 1)
    dependency = "      mother-super-node-hub:\n        condition: service_healthy"
    replacement = "      mother-super-node-hub:\n        condition: service_started"
    if guardian.count(dependency) != 1:
        raise _fail(
            "MOTHER_DEPLOY_NODE_ADD_SINGLE_NODE_BOOTSTRAP_GUARDIAN_DEPENDENCY_INVALID",
            "single-node bootstrap guardian must have exactly one Hub service_healthy dependency",
        )
    guardian = guardian.replace(dependency, replacement, 1)
    return before + marker + guardian + volumes_marker + after
...
def build_node_add_single_node_bootstrap_release(
    ...
    *,
    ...
    proof_compose = _bootstrap_v2_proof_compose(proof_compose)
    ...
    release: dict[str, Any] = {
        ...
        "bootstrap_plan": {
            ...
            "mutations": [
                ...
                {"ordinal": 2, "method": "POST", "endpoint": "/api/v1/deploy", "canonical_request_body": {"uuid": service_uuid, "force": True}, "body_sha256": hashlib.sha256(canonical_json({"uuid": service_uuid, "force": True})).hexdigest(), "success_statuses": [200, 201, 202]},
            ],
            "proof_required": [
        ...
        "policy": {
            "compiler": "mother-native-add-node-single-node-bootstrap-v2",
