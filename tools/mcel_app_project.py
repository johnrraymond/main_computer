#!/usr/bin/env python3
"""Project a DSL-authored MCEL application through the generic app pipeline."""
from __future__ import annotations
import argparse, json, sys
from pathlib import Path
if __package__ in {None, ""}: sys.path.insert(0, str(Path(__file__).resolve().parents[1]))
from main_computer.mcel_app_project import AppProjectionError, project_application
...
def main(argv=None):
    ...
    a=p.parse_args(argv)
    try: r=project_application(app_id=a.app,repo_root=a.repo_root,candidate_root=a.candidate_root,write_candidate=a.write_candidate)
    except (AppProjectionError, RuntimeError) as exc:
        print(f"mcel-app-project-wave9\napp: {a.app}\nstatus: fail\ndiagnostics: 1\n  MCEL_APP_PROJECT_FAILED: {exc}")
        return 2
    d=r.to_dict()
    if a.json: print(json.dumps(d,indent=2,sort_keys=True,ensure_ascii=False))
    else:
        print("mcel-app-project-wave9")
        print(f"app: {a.app}")
        print(f"status: {r.status}")
        blocking=sum(1 for item in r.diagnostics if item.get("blocking",True))
        warnings=int((((d.get("projection") or {}).get("source") or {}).get("opaqueCallbackDebt")) or 0)
        ...
        print(f"projection_profile: {d.get('projectionProfile')}")
