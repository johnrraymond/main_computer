...
def execute_genesis_proof_guardian_cleanup(
    ...
    *,
    ...
    if instant_deploy:
        raise MotherGenesisProofGuardianCleanupError(
            "MOTHER_GENESIS_PROOF_GUARDIAN_CLEANUP_PARENT_DEPLOY_FORBIDDEN",
    ...
    _emit_progress(
        progress,
        "execute.compose",
        "rewriting retired genesis proof guardian mimic compose",
        service_uuid=service,
    ...
    if patch_receipt.get("ok"):
        _emit_progress(
            ...
            "restarting existing genesis proof guardian helper child only",
            service_uuid=service,
            helper_service=SHIM_NAME,
            target_application_uuid=target_application_uuid,
        )
