...
def compact_grounding_schema() -> dict[str, Any]:
    return {
        ...
        "request_scope": {
            "operation_class": "localized_text_replacement | localized_style_change | localized_behavior_change | create_new_content | broader_refactor_requested",
            "authorized_change": "smallest source region allowed to change",
            "forbidden_collateral_changes": [
                "delete sibling content",
                "rewrite surrounding container",
                "remove metadata/imports/assets/scripts/styles",
                "cleanup unrelated code or markup",
            ],
        },
        "preserve": [
            {
...
- Classify request_scope.operation_class before allowing generation.
- For localized copy/text/content/label/style/behavior edits, authorize only the
  smallest existing source region that must change. Treat all sibling content,
  surrounding containers, metadata/list rows, imports/assets/scripts/styles,
  attributes, and unrelated code as preservation invariants unless the task
  explicitly asks to change them.
- The preservation checks should mechanically catch collateral deletion or
  rewriting of nearby unrelated source.
- Do not allow generation for cleanup, deletion, reordering, redesign, or
  refactoring unless the task explicitly asks for that work.
...
- Do not use keys not present in the required shape except request_scope if present in the shape.
- Include one new_behavior check and one preservation check.
- Preserve request scope: for localized edits, authorize only the smallest existing
  source region that must change and add preservation checks for unrelated nearby
  source.  Do not authorize sibling deletions, container rewrites, cleanup,
  reordering, redesign, or refactoring unless the task explicitly asks.
...
- For localized copy/text/content/label/style/behavior requests, only modify the
  authorized target text/rule/symbol. Preserve every other byte of SOURCE unless
  a grounding check explicitly authorizes that exact collateral change.
...
- A cleaner-looking rewrite is not allowed. If the task cannot be satisfied
  without collateral changes, return a proposal that changes nothing and relies
  on the grounding card to block promotion.
