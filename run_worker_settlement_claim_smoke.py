...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        claim_idempotent = bool(claimed.get("idempotent", False))
        if claimable_before:
            assert_equal("claimed units", claimed_units, args.worker_credits)
            assert_equal("claimed count", int(claimed.get("claimed_count", 0)), 1)
            claim_additional_units = claimed_units
        else:
            # A same-scope rerun POSTs the same idempotency key after the claim already
            # exists. The API correctly returns the original durable claim amount; that is
            # not a newly paid amount.
            assert_equal("rerun claim is idempotent", claim_idempotent, True)
            assert_equal("rerun returned existing claim units", claimed_units, args.worker_credits)
            assert_equal("rerun claimed count", int(claimed.get("claimed_count", 0)), 1)
            claim_additional_units = 0
        ...
        duplicate_same_additional_units = 0
        ...
        report.update(
            {
                ...
                "claim_additional_units_this_run": claim_additional_units,
                "record_worker_claim_idempotent": claim_idempotent,
                ...
                "duplicate_same_key_additional_units": duplicate_same_additional_units,
    ...
    if args.json:
        ...
        print(
            "summary: "
            f"earning={report['earning_units']} claimed_returned={report['claimed_units']} "
            f"claim_additional={report['claim_additional_units_this_run']} "
