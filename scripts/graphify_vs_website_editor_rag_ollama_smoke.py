...
def run_contract_tests(repo: Path, timeout: float) -> dict[str, Any]:
    tests = [
        "tests/test_debug_website_golden_path_no_deterministic_cheats.py",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.skip_contract_tests:
            ...
            report["contract_tests"] = run_contract_tests(repo, args.contract_test_timeout)
            if not report["contract_tests"]["ok"]:
                raise BenchmarkFailure(
                    "Website Builder/anti-cheat contract tests failed:\n"
                    + (report["contract_tests"]["stderr"] or report["contract_tests"]["stdout"])
                )
