...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Phase 7a operator settlement proof smoke.")
    ...
    parser.add_argument("--report-path", default="runtime/hub/worker_operator_settlement_proof_smoke.json")
    ...
    requester = f"phase7a-operator-settlement-requester-{args.requester_index}-{scope}"
    worker = f"paid-mock-worker-phase7a-proof-{args.worker_index}-{scope}"
    request_key = f"phase7a-operator-settlement-request-{scope}"
    claim_key = f"phase7a-operator-settlement-claim-{scope}"
    batch_key = f"phase7a-operator-settlement-batch-{scope}"
    settle_key = f"phase7a-operator-settlement-proof-{scope}"
    ...
    report: dict = {
        "ok": False,
        "phase": "phase7a-operator-settlement-proof",
        "hub_url": hub_url,
        "scope": scope,
        "requester": {"account_id": requester, "self_contained_phase7a": True},
        "worker": {"node_id": worker, "self_contained_phase7a": True},
    ...
    try:
        ...
        issued = add_step(
            ...
            post_json(
                f"{hub_url}/api/hub/v1/credits/admin/issue",
                {
                    ...
                    "memo": "phase7a operator settlement proof smoke funding",
                    "metadata": {"phase7a_operator_settlement_proof": True, "scope": scope},
                },
            ),
        )
            ...
            post_json(
                f"{hub_url}/api/hub/v1/workers/register",
                {
                    ...
                    "capabilities": {
                        "provider": "mock",
                        "worker_pull_v0": True,
                        "phase7a_operator_settlement_proof": True,
                    },
                },
            ),
        )
        ...
        submitted = add_step(
            ...
            post_json(
                f"{hub_url}/api/hub/v1/requests",
                {
                    ...
                    "model": "mock-fast-chat",
                    "prompt": "phase7a operator settlement proof smoke",
                    "max_credits": args.max_credits,
                    "execution_mode": "worker_pull_v0",
                    "metadata": {
                        "worker_pull_v0": True,
                        "phase7a_operator_settlement_proof": True,
                        "mock_provider_config": {"answer": "phase7a operator settlement proof answer"},
                    },
                ...
                post_json(
                    f"{hub_url}/api/hub/v1/workers/results",
                    {
                        ...
                        "result": {
                            "status": "success",
                            "response": {
                                "content": "phase7a operator settlement proof answer",
        ...
        claim = add_step(
            ...
            post_json(
                f"{hub_url}/api/hub/v1/workers/claims",
                {
                    ...
                    "memo": "phase7a worker proof claim",
                    "metadata": {"phase7a_operator_settlement_proof": True, "scope": scope},
                },
            ),
        )
        ...
        proof_payload = {
            ...
            "precision_places": args.precision_places,
            "operator_receipt": f"manual-proof-{scope}",
        }
        settled = add_step(
            steps,
            "record operator settlement proof",
            post_json(
                f"{hub_url}/api/hub/v1/workers/settlements/proofs",
                {
                    "batch_id": batch_payload["batch_id"],
                    "settlement_reference": f"phase7a-operator-proof-{scope}",
                    "payout_rail": "operator-manual",
                    "operator_id": f"phase7a-operator-{scope}",
        ...
        if not proof_id.startswith("proof_"):
            raise AssertionError(f"operator proof id missing: {proof_id!r}")
        assert_equal("proof hash length", len(proof_hash), 64)
        assert_equal("payout rail", str(settled_batch.get("payout_rail", "")), "operator-manual")

        duplicate_settle = add_step(
            steps,
            "duplicate settlement proof does not pay again",
            post_json(
                f"{hub_url}/api/hub/v1/workers/settlements/proofs",
                {
                    "batch_id": batch_payload["batch_id"],
        ...
        report.update(
            {
                ...
                "operator_settlement_proof_id": proof_id,
                "operator_settlement_proof_hash": proof_hash,
                "payout_rail": "operator-manual",
        ...
        if args.json:
            ...
            print(
                "ok phase7a operator settlement proof: "
                f"exact={args.worker_credits} visible={expected_published} "
                f"bridge_retained={expected_dust} normal_leak=false proof=true duplicate_additional=0"
            )
        return 0
        ...
        if args.json:
            print(json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True))
        print(f"phase7a operator settlement proof smoke failed: {exc}", file=sys.stderr)
        return 1
