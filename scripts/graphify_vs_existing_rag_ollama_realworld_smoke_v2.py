...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--min-grounded-score",
        "--min-graphify-grounded-score",
        dest="min_graphify_grounded_score",
        type=float,
        default=0.55,
        help=(
            "Minimum average grounded score required from the Graphify+Ollama "
            "candidate lane. --min-grounded-score remains a compatibility alias."
        ),
    )
    parser.add_argument(
        "--min-existing-grounded-score",
        type=float,
        default=0.0,
        help=(
            "Optional minimum average grounded score required from the existing "
            "RAG+Ollama baseline lane. Defaults to 0.0 so a weak baseline is "
            "measured rather than causing the comparative benchmark to fail."
        ),
    )
    parser.add_argument(
        "--require-graphify-wins",
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if not 0.0 <= float(args.min_graphify_grounded_score) <= 1.0:
        raise SmokeFailure(
            "--min-grounded-score/--min-graphify-grounded-score must be between 0 and 1."
        )
    if not 0.0 <= float(args.min_existing_grounded_score) <= 1.0:
        raise SmokeFailure("--min-existing-grounded-score must be between 0 and 1.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report: dict[str, Any] = {
        ...
        "quality_gates": {
            "graphify_minimum_grounded_score": args.min_graphify_grounded_score,
            "existing_rag_minimum_grounded_score": args.min_existing_grounded_score,
            "require_graphify_wins": bool(args.require_graphify_wins),
        },
    ...
    try:
        ...
        if not infrastructure_ok:
            ...
            exit_code = 2
        elif right_score < args.min_graphify_grounded_score:
            report["status"] = "failed_graphify_minimum_grounded_score"
            warnings.append(
                "Graphify+Ollama scored "
                f"{right_score:.4f}, below the required minimum "
                f"{args.min_graphify_grounded_score:.4f}."
            )
            exit_code = 2
        elif left_score < args.min_existing_grounded_score:
            report["status"] = "failed_existing_rag_minimum_grounded_score"
            warnings.append(
                "Existing RAG+Ollama scored "
                f"{left_score:.4f}, below the optional baseline minimum "
                f"{args.min_existing_grounded_score:.4f}."
            )
            exit_code = 2
