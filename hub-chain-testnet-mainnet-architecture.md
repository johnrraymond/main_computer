...
RPC sidecar, so the default `testnet` Coolify seed intentionally uses a validator
that also serves the operator/public RPC path. This proves the Hub/RPC/contract
path but is not fault-tolerant.

For rehearsal, the remote testnet can temporarily add a second validator-only
node on the same host:

```text
two-validator rehearsal state

validator-rpc-1  <---- peer ---->  validator-1
  validator + rpc                  validator only
      |
      v
Hub server / Hub site / operator tools
```

The important distinction is that a running Besu peer is not automatically a
QBFT validator. The authoritative state is the output of
`tools/coolify_qbft_network.py observe-chain testnet`: `peer_count` describes
connected peers, while `validator_count` and `validators` describe the consensus
signer set. In the one-validator state, `peer_count: 0` is normal. In the
two-validator state, `peer_count: 1` is normal.

Operators move between those two shapes with consensus-aware mutation commands:

```powershell
python .\tools\coolify_qbft_network.py mutate testnet --add validator-1 --apply --ack-consensus-change --rpc-timeout-s 60 --chain-observation-timeout-s 600 --config-export-timeout-s 600
python .\tools\coolify_qbft_network.py mutate testnet --retire validator-1 --apply --ack-consensus-change --rpc-timeout-s 60 --chain-observation-timeout-s 600 --config-export-timeout-s 600
```

Those commands vote validators into or out of QBFT before treating the topology as
committed. Do not manually delete validator containers as a substitute for
`mutate --retire`.

Contract deployment remains separate. If the chain data volume is reset or the
block history starts over, redeploy contracts and refresh
`main_computer/config/testnet_contracts.json`. Adding or retiring `validator-1`
without resetting chain history does not by itself invalidate contract addresses.
...
```text
current low-resource testnet can run one validator-RPC node
operator can add validator-1 into QBFT consensus and verify validator_count: 2
operator can retire validator-1 and verify the one-validator chain still produces blocks
contract deployment is repeated after chain resets and preserved across validator membership changes
future promoted testnet moves validators onto separate hosts and separates public RPC from validators
