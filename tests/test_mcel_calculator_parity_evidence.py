...
def test_calculator_generated_adapter_is_authoritative_without_legacy_semantic_adapter() -> None:
    ...
    assert payload["checks"]["manifestAuthoritative"] is True
    assert payload["checks"]["legacyAdapterRetired"] is True
    assert payload["checks"]["legacySurfaceRetired"] is True
    assert payload["checks"]["runtimeProjectionHostBound"] is True
    assert payload["checks"]["browserCatalogHostBound"] is True
    assert payload["checks"]["generatedRuntimeBindingsExact"] is True
...
def test_calculator_fresh_browser_observation_exercises_generated_adapter_authority() -> None:
    ...
    assert payload["checks"]["runtimeFacadeAvailable"] is True
    assert payload["checks"]["intentsObserved"] is True
    assert payload["checks"]["generatedRuntimeParityStatus"] is True
...
def test_calculator_authoring_profile_wires_authoritative_evidence_hooks() -> None:
...
def test_calculator_authoritative_ir_native_proof_converges_from_browser_evidence() -> None:
    ...
    assert proof["status"] == "ir-native-authoritative"
    assert proof["coverageMode"] == "fresh-browser-dsl-authoritative-host-bound-generated-adapter"
    ...
    assert proof["crossCuttingChecks"]["legacyAdapterRetired"] is True
    assert proof["parityEvidence"]["freshChromiumObservation"] is True
    assert proof["promotionEligible"] is True


def test_calculator_candidate_evidence_isolated_authoritative_and_write_free(tmp_path: Path) -> None:
    ...
    assert payload["status"] == "pass"
    assert payload["truthStatus"] == "fresh-browser-dsl-authoritative-ir-native"
