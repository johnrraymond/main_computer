...
def test_lifecycle_plan_defaults_to_site_compose_and_registry_aliases(tmp_path: Path) -> None:
...
def test_lifecycle_plan_can_opt_into_legacy_aggregate_compose(tmp_path: Path) -> None:
    list_website_projects(tmp_path)

    plan = lifecycle_plan(tmp_path, "hub-site", lane="dev", action="start", compose_scope="aggregate")

    assert plan.compose_scope == "aggregate"
...
def test_install_registers_missing_site_allocates_ports_and_writes_site_compose(tmp_path: Path) -> None:
    ...
    assert result["compose_scope"] == "site"
    ...
    compose_path = tmp_path / "runtime" / "websites" / "portfolio-site" / ".main-computer" / "local-platform" / "docker-compose.yml"
    aggregate_compose_path = tmp_path / "deploy" / "local-platform" / "generated" / "docker-compose.websites.yml"
    compose_text = compose_path.read_text(encoding="utf-8")
    assert 'name: "main-computer-website-portfolio-site"' in compose_text
...
def test_start_dry_run_defaults_to_site_compose_without_running_docker(tmp_path: Path) -> None:
    list_website_projects(tmp_path)
...
def test_start_dry_run_can_still_use_legacy_aggregate_compose(tmp_path: Path) -> None:
    list_website_projects(tmp_path)
    ...
    assert "docker-compose.websites.yml" in result["plan"]["compose_path"]
    assert result["plan"]["compose_project"] == "main-computer-local-platform-unleashed"
    assert result["plan"]["command"][-4:] == ["up", "-d", "--build", "hub-dev"]
...
def test_publish_runs_site_compose_verifies_and_marks_manifest(tmp_path: Path, monkeypatch) -> None:
    list_website_projects(tmp_path)
    calls: list[list[str]] = []
    ...
    assert "main-computer-website-hub-site" in calls[0]
    assert any(".main-computer" in part for part in calls[0])
    assert not any("docker-compose.websites.yml" in part for part in calls[0])
...
def test_stop_and_logs_dry_runs_target_only_selected_site_service(tmp_path: Path) -> None:
    list_website_projects(tmp_path)
    ...
    assert stop_result["plan"]["compose_scope"] == "site"
    assert stop_result["plan"]["compose_project"] == "main-computer-website-blog-site"
    assert stop_result["plan"]["service"] == "blog-local"
    assert stop_result["plan"]["command"][-2:] == ["stop", "blog-local"]
    assert logs_result["plan"]["compose_scope"] == "site"
    assert logs_result["plan"]["compose_project"] == "main-computer-website-blog-site"
...
def test_website_docker_cli_dry_run_outputs_site_scoped_json_by_default(tmp_path: Path) -> None:
    list_website_projects(tmp_path)
...
def test_website_docker_cli_can_use_legacy_aggregate_scope(tmp_path: Path) -> None:
    list_website_projects(tmp_path)
    script = Path("tools") / "local-platform" / "website-docker.py"
    ...
    payload = json.loads(completed.stdout)
    assert payload["ok"] is True
    assert payload["plan"]["compose_scope"] == "aggregate"
    assert payload["plan"]["compose_project"] == "main-computer-local-platform-unleashed"
