...
class ViewportGameEditorTests(unittest.TestCase):
    ...
    def test_projects_list_and_read_return_webgl_demo_default_with_hash(self) -> None:
        ...
        self.assertEqual(scene["metadata"]["rolloutPhase"], "phase-5-shuttle-boarding-combat")
        self.assertIn("mother ship", scene["metadata"]["setting"])
        self.assertEqual(scene["metadata"]["characterModel"], "first-person-cadet-combat-presence")
        self.assertFalse(scene["metadata"]["meshActorsEnabled"])
        self.assertEqual(scene["metadata"]["controls"]["mode"], "first-person")
        self.assertTrue(scene["metadata"]["controls"]["pointerDrag"])
        self.assertEqual(scene["metadata"]["controls"]["keyboard"], "wasd-arrows-space-fire")
        ...
        self.assertEqual(scene["metadata"]["controls"]["fire"], "click-space-or-f")
        self.assertEqual(scene["metadata"]["controls"]["restart"], "r")
        self.assertEqual(scene["metadata"]["controls"]["interact"], "mouse-over-console-e-and-ship-interactions")
        self.assertEqual(scene["metadata"]["controls"]["shipInterior"], "post-docking-walkable-mother-ship-decks")
        self.assertEqual(scene["metadata"]["controls"]["pilot"], "console-hover-e-flies-to-mother-ship-docking-cutscene-then-shuttle-bay-walk")
        self.assertEqual(scene["metadata"]["controls"]["postDocking"], "first-person-shuttle-bay-control")
        self.assertEqual(scene["metadata"]["shuttle3d"]["mode"], "webgl-vertex-mesh")
        flight = scene["metadata"]["shuttle3d"]["flight"]
        self.assertTrue(flight["enabled"])
        self.assertEqual(flight["targetLabel"], "Mother Ship")
        self.assertEqual(flight["startDistance"], 36.5)
        self.assertEqual(flight["dockingDistance"], 11.5)
        self.assertEqual(flight["maxForwardSpeed"], 12.5)
        self.assertTrue(flight["cutscene"]["enabled"])
        self.assertEqual(flight["cutscene"]["durationMs"], 9600)
        self.assertEqual(flight["cutscene"]["bayLabel"], "Mother Ship Shuttle Bay")
        interior = scene["metadata"]["shuttle3d"]["motherShipInterior"]
        self.assertTrue(interior["enabled"])
        self.assertEqual(interior["initialLocation"], "bay.shuttle")
        self.assertEqual(interior["initialObjective"], "objective.bay-ops")
        self.assertEqual(interior["objectives"]["objective.bay-ops"]["label"], "Use the starboard interior access and bring Bay Operations online.")
        self.assertEqual(interior["power"], "emergency")
        self.assertEqual(interior["security"], "quarantine")
        self.assertEqual(interior["locations"]["bay.shuttle"], "Mother Ship Shuttle Bay")
        self.assertEqual(interior["doors"]["door.bay-access"]["state"], "open")
        self.assertEqual(interior["doors"]["door.bay-access"]["from"], "bay.shuttle")
        self.assertEqual(interior["doors"]["door.bay-access"]["to"], "bay.ops")
        self.assertEqual(interior["doors"]["door.bay-inner"]["state"], "open")
        self.assertEqual(interior["terminals"]["terminal.bay-ops"]["location"], "bay.ops")
        self.assertEqual(interior["locations"]["engineering.access"], "Engineering Access")
        self.assertEqual(interior["locations"]["medbay.stub"], "Medbay Triage")
        self.assertEqual(interior["locations"]["science.ops.stub"], "Science/Ops Lab")
        self.assertEqual(interior["locations"]["bridge.access"], "Bridge Access")
        self.assertEqual(interior["locations"]["bridge.deck"], "Bridge Deck")
        self.assertEqual(interior["rooms"][0]["id"], "bay.shuttle")
        self.assertEqual(interior["rooms"][0]["location"], "bay.shuttle")
        self.assertEqual(interior["rooms"][0]["bounds"]["maxZ"], 5.12)
        self.assertEqual(interior["rooms"][-1]["id"], "bridge.deck")
        ...
        self.assertEqual(interior["spawns"]["spawn.shuttle-bay"]["pitch"], -4)
        self.assertEqual(interior["movement"]["bounds"]["minZ"], -39.65)
        self.assertEqual(interior["movement"]["colliders"][0]["id"], "docked-shuttle-hull")
        self.assertEqual(interior["exits"][-1]["id"], "exit.bridge-deck")
        self.assertEqual(interior["exits"][-1]["to"], "bridge.deck")
        self.assertEqual(interior["doors"]["door.bridge"]["state"], "open")
        self.assertEqual(interior["doors"]["door.bridge"]["to"], "bridge.deck")
        self.assertEqual(interior["objectives"]["objective.restore-power"]["location"], "corridor.main")
        self.assertEqual(interior["objectives"]["objective.bridge-screen"]["location"], "bridge.deck")
        self.assertEqual(interior["objectives"]["objective.enemy-track"]["location"], "bridge.deck")
        self.assertEqual(interior["objectives"]["objective.enemy-attack"]["location"], "bridge.deck")
        self.assertEqual(interior["objectives"]["objective.enemy-disabled"]["location"], "bridge.deck")
        self.assertEqual(interior["terminals"]["terminal.engineering-power"]["location"], "engineering.access")
        self.assertEqual(interior["terminals"]["terminal.bridge-viewscreen"]["location"], "bridge.deck")
        ...
        self.assertEqual(len(pilot_stations), 4)
        self.assertEqual(pilot_stations[0]["objectId"], "nav-console")
        self.assertEqual(pilot_stations[0]["camera"]["position"], [-0.92, 0.82, -2.82])
        self.assertEqual(pilot_stations[0]["activationRange"], 3.35)
        ...
        combat = scene["metadata"]["shuttle3d"]["combat"]
        self.assertTrue(combat["enabled"])
        self.assertEqual(combat["player"]["maxHealth"], 100)
        self.assertEqual(combat["phaser"]["damage"], 34)
        self.assertEqual(combat["transport"]["maxAlive"], 4)
        self.assertEqual(len(combat["transport"]["spawnPoints"]), 4)
        self.assertEqual(combat["alien"]["maxHealth"], 60)
        self.assertEqual(combat["alienShip"]["id"], "alien-raider")
        ...
        self.assertEqual(geometry["boundsVertexCount"], 12)
        self.assertEqual(len(geometry["boundsVertices"]), 12)
        self.assertEqual(geometry["cabinLength"], 12.0)
        self.assertEqual(geometry["boundsVertices"][0][2], -7.2)
        self.assertEqual(geometry["boundsVertices"][6][2], 4.8)
        self.assertTrue(geometry["actualHullBounds"])
        movement = scene["metadata"]["shuttle3d"]["movement"]
        self.assertTrue(movement["enabled"])
        self.assertEqual(movement["scheme"], "wasd")
        self.assertEqual(movement["start"], [0.0, 0.75, 2.45])
        self.assertEqual(movement["bounds"]["minZ"], -6.12)
        self.assertGreaterEqual(len(movement["colliders"]), 7)
        self.assertEqual(scene["metadata"]["camera"]["mode"], "first-person")
        self.assertEqual(scene["metadata"]["camera"]["position"], [0.0, 0.75, 2.45])
        self.assertEqual(scene["metadata"]["camera"]["yawLimit"], 180)
        self.assertEqual(scene["metadata"]["camera"]["pitchLimit"], 28)
        self.assertTrue(scene["metadata"]["lookAroundEnabled"])
        self.assertTrue(scene["metadata"]["parentedParticles"])
        self.assertTrue(scene["metadata"]["linkedSpellProjectiles"])
        ...
        self.assertEqual(objects["nav-console"]["props"]["interaction"], "pilot-shuttle")
        self.assertEqual(objects["nav-console"]["props"]["interactKey"], "E")
        self.assertTrue(objects["nav-console"]["props"]["pausesCombat"])
        self.assertEqual(objects["nav-console"]["props"]["pilotStation"], "helm-console")
        self.assertEqual(objects["starboard-side-console"]["props"]["pilotStation"], "starboard-console")
        ...
        self.assertEqual(objects["alien-raider"]["type"], "shuttle3d-alien-ship")
        self.assertTrue(objects["alien-raider"]["props"]["visibleThroughViewport"])
        self.assertEqual(objects["player-phaser"]["type"], "shuttle3d-phaser")
        self.assertEqual(objects["boarding-transporter"]["type"], "shuttle3d-transporter")
        self.assertEqual(objects["player-health-hud"]["props"]["maximum"], 100)
        ...
        self.assertEqual(scene["metadata"]["choreography"]["title"], "Shuttle Boarding Alert")
    ...
    def test_gpu_forge_frontend_links_baked_atlas_for_scene_viewer_playback(self) -> None:
        ...
        self.assertIn("shuttle3dMotherShipInteriorLevelDefaults()", scene_viewer_script)
        self.assertIn("game.motherShipInterior.level.v1", scene_viewer_script)
        self.assertIn("shuttle3dNormalizeMotherShipRooms", scene_viewer_script)
        self.assertIn("shuttle3dMovementBoundsFromRooms", scene_viewer_script)
        self.assertIn("shuttle3dNormalizeMotherShipInteractables", scene_viewer_script)
        self.assertIn("Patch D makes E-key targets data-driven", scene_viewer_script)
        self.assertIn("Patch E formalizes action ids as registry entries", scene_viewer_script)
        self.assertIn("shuttle3dNormalizeMotherShipInteractions", scene_viewer_script)
        self.assertIn("createShipInteractionRegistry()", scene_viewer_script)
        self.assertIn("createShipInteractionHandlerMap()", scene_viewer_script)
    ...
    def test_frontend_static_hooks_and_routes(self) -> None:
        ...
        self.assertIn("Vertex-built shuttle boarding-defense surface", APPLICATIONS_INDEX_HTML)
        ...
        self.assertIn("buildDynamicGeometry(nowMs)", APPLICATIONS_INDEX_HTML)
        ...
        self.assertIn("scene-shuttle3d-crosshair", APPLICATIONS_INDEX_HTML)
