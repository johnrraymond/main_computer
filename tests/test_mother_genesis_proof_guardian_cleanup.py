...
from tools.mother_genesis_proof_guardian_cleanup import (
    MotherGenesisProofGuardianCleanupError,
    SHIM_NAME,
...
def _install(tmp_path: Path):
    ...
    operation = _operation("genesis-proof-guardian-cleanup-install")
    ...
    private_state = read_private_state(paths, operation=_operation("genesis-proof-guardian-cleanup-read"))
...
"""
        if self.shim_installed:
            ...
            compose_obj["services"]["mother-genesis-proof-guardian"] = {
                "image": "alpine:3.20",
                "container_name": f"mother-genesis-proof-guardian-{SERVICE_UUID}",
                "labels": {
    ...
    def _payload(self) -> dict[str, object]:
        applications = [
            ...
            _application("mother-genesis-init", "genesisinit123", "exited", "alpine:3.20"),
            _application(
                "mother-add-node-validator-admission-voter-mainnetc-super1",
                "admissionvoter123",
        ...
        if self.shim_installed:
            ...
            applications.append(_application("mother-genesis-proof-guardian", "genesisguardian123", "running:unhealthy"))
        return {
    ...
    def open(self, request, timeout: float):  # noqa: ANN001
        ...
        if method == "PATCH" and path == f"/api/v1/services/{SERVICE_UUID}":
            ...
            assert "mother-genesis-init" in services
        ...
        if method == "POST" and path == "/api/v1/applications/genesisguardian123/restart":
            return _Response({"ok": True, "uuid": "genesisguardian123", "action": "restart"})
        raise AssertionError(f"unexpected request: {method} {path}")


def test_inspect_reports_genesis_proof_guardian_cleanup_required_without_mutation(tmp_path: Path) -> None:
    _, private_state = _install(tmp_path)
    opener = _GenesisShimOpener()
...
def test_execute_installs_only_minimal_retired_genesis_proof_guardian_shim(tmp_path: Path) -> None:
    _, private_state = _install(tmp_path)
    opener = _GenesisShimOpener()
    ...
    assert result["summary"]["retired_genesis_proof_guardian_shim_serves_proof"] is False
    assert result["summary"]["retired_genesis_proof_guardian_shim_mounts_proof_volume"] is False
    assert result["docker_touched"] is True
    assert result["chain_touched"] is False
    assert ("PATCH", f"/api/v1/services/{SERVICE_UUID}") in opener.requests
    assert ("POST", "/api/v1/applications/genesisguardian123/restart") in opener.requests
...
def test_execute_refuses_without_explicit_shim_flag(tmp_path: Path) -> None:
    ...
    with pytest.raises(MotherGenesisProofGuardianCleanupError) as exc:
        execute_genesis_proof_guardian_cleanup(
            private_state,
            network="mainnet",
            controller_id="coolify-c",
            ...
            node="mainnetc-super1",
            acknowledged_service_uuid=SERVICE_UUID,
            allow_retired_genesis_proof_guardian_shim=False,
            opener=opener,
        )

    assert exc.value.code == "MOTHER_GENESIS_PROOF_GUARDIAN_CLEANUP_SHIM_FLAG_REQUIRED"
...
def test_execute_refuses_wrong_service_ack(tmp_path: Path) -> None:
    ...
    assert exc.value.code == "MOTHER_GENESIS_PROOF_GUARDIAN_CLEANUP_ACK_REQUIRED"
    assert opener.requests == []
