...
severity: blocking
problem: The generated package is structurally valid, discoverable, executable through the shared SCM-controlled application runtime, and mountable through generic semantic-surface projection, but package-local acceptance discovery, operation-linked browser observation, and app-oriented proof orchestration are not yet live.
...
required_intents:
  ...
  - reset
  - direct-set refusal
readiness_gate: Bind the mounted SCM-controlled operation results to package-local acceptance, operation-linked observation, and later repository-bound proof.
```
