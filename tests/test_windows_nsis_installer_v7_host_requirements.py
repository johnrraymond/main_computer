...
def test_v7_keeps_mode_shortcut_open_browser_contract() -> None:
    ...
    assert "Choose the Main Computer install mode and container runtime." in text
    assert "Docker-compatible container runtime" in text
    assert '${NSD_CreateRadioButton} 0 134u 100% 12u "Auto-detect on this computer"' in text
    assert '${NSD_CreateRadioButton} 0 152u 100% 12u "Docker Desktop"' in text
    assert '${NSD_CreateRadioButton} 0 170u 100% 12u "Podman"' in text
...
def test_v7_hard_gates_selected_container_runtime_with_runtime_specific_guidance() -> None:
    ...
    assert "Page custom ModePage ModePageLeave" in text
    assert "Page custom ContainerRuntimePage ContainerRuntimePageLeave" not in text
    assert "Function ContainerRuntimePage" not in text
...
def test_v7_collects_mode_and_runtime_before_instfiles_on_one_page() -> None:
    text = read_builder()

    page_order = text.index("Page custom ModePage ModePageLeave")
    instfiles_order = text.index("Page instfiles")
    section_order = text.index('Section "Install Main Computer package files"')

    assert page_order < instfiles_order < section_order
    assert "Page custom ContainerRuntimePage" not in text
    assert text.index('"Docker-compatible container runtime"') < text.index("nsDialogs::Show")
    assert text.index('${NSD_GetState} $ContainerRuntimeRadioPodman $0') < text.index("Call ResolveSelectedInstallRoot")
