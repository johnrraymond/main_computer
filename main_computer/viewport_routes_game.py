...
_LEGACY_OPENING_SHUTTLE_PLUGIN_ID = "plugin.hand-authored.opening-shuttle-ambush.001"
_JS_OPENING_SHUTTLE_PACK_DIR = "opening_shuttle_elite_boarders"
_JS_MAIN_SHIP_BAY_BOARDERS_PACK_ID = "pack.main-ship.bay-boarders"
_JS_MAIN_SHIP_BAY_BOARDERS_PACK_DIR = "main_ship_bay_boarders"
_JS_GAMEPLAY_PACKS = {
    _JS_OPENING_SHUTTLE_PACK_ID: {
        "directory": _JS_OPENING_SHUTTLE_PACK_DIR,
        "title": "Opening Shuttle: Elite Boarders",
        "aliases": {_LEGACY_OPENING_SHUTTLE_PLUGIN_ID},
    },
    _JS_MAIN_SHIP_BAY_BOARDERS_PACK_ID: {
        "directory": _JS_MAIN_SHIP_BAY_BOARDERS_PACK_DIR,
...
class ViewportGameRoutesMixin:
    def _handle_game_editor_post(self) -> None:
        try:
            ...
            if route == "/api/applications/game-editor/gameplay-pack/load":
                ...
                return
            if route == "/api/applications/game-editor/gameplay-pack/js-source":
                self._send_json(self._game_gameplay_pack_js_source_payload(body))
                return
        ...
        for path in visible_files:
            ...
            if rel == "project.json":
                if re.search(r"\b(color|colour|tint|red|blue|green|yellow|purple|orange|white|black|sprite|visual|main|character|char|hero|player|scene|object|movement|jump|control|vfx|particle|metadata|project|settings|spawn)\b", source_text):
                    score += 10
                    reason = "project.json contains active scene objects and editable object props such as Player Cadet/player props.color."
                else:
                    score += 1
    ...
    def _game_mother_ship_default_interior(self) -> dict[str, Any]:
        try:
            default_file = self._game_project_root(self._default_game_project_id()) / "project.json"
            project = json.loads(default_file.read_text(encoding="utf-8"))
            for scene in project.get("scenes", []):
                ...
                if isinstance(interior, dict):
                    return json.loads(json.dumps(interior))
        except Exception:
            pass
        return {
            "schema": "game.motherShipInterior.v1",
            ...
            "power": "emergency",
            "security": "quarantine",
            "locations": {"bay.shuttle": "Mother Ship Shuttle Bay"},
            "rooms": [
                {
                    "id": "bay.shuttle",
                    "name": "Mother Ship Shuttle Bay",
            ...
            "exits": [],
            "spawns": {"spawn.shuttle-bay": {"room": "bay.shuttle", "position": [0.24, 0.9, 4.3]}},
            "movement": {"bounds": {"minX": -4.72, "maxX": 4.72, "minZ": -4.62, "maxZ": 5.12}, "colliders": []},
            "objectives": {"objective.bay-ops": {"label": "Use Bay Operations.", "location": "bay.shuttle"}},
            "doors": {},
            "terminals": {},
            "flags": {},
            "interactables": [],
            "interactions": {},
            "validation": {"requireDefinitionVersion": True, "requireOpenDoors": True},
            "props": [],
        }

    def _game_migrate_mother_ship_interior_for_read(self, interior: dict[str, Any], fallback: dict[str, Any]) -> dict[str, Any]:
        target = json.loads(json.dumps(interior if isinstance(interior, dict) else {}))
        defaults = json.loads(json.dumps(fallback if isinstance(fallback, dict) else {}))
        defaults_applied: list[str] = []

        def scalar_default(key: str, value: Any) -> None:
            if target.get(key) in (None, ""):
                target[key] = json.loads(json.dumps(value))
                defaults_applied.append(key)

        def object_default(key: str) -> None:
            supplied = target.get(key)
            fallback_value = defaults.get(key, {})
            if not isinstance(supplied, dict) or not supplied:
                target[key] = json.loads(json.dumps(fallback_value if isinstance(fallback_value, dict) else {}))
                defaults_applied.append(key)
                return
            if isinstance(fallback_value, dict):
                merged = json.loads(json.dumps(fallback_value))
                merged.update(supplied)
                target[key] = merged

        def list_default(key: str) -> None:
            supplied = target.get(key)
            fallback_value = defaults.get(key, [])
            if not isinstance(supplied, list) or not supplied:
                target[key] = json.loads(json.dumps(fallback_value if isinstance(fallback_value, list) else []))
                defaults_applied.append(key)

        source_version = str(target.get("definitionVersion") or target.get("definition_version") or target.get("version") or target.get("schema") or "legacy-unversioned")
        target["schema"] = "game.motherShipInterior.v1"
        target["definitionVersion"] = "game.motherShipInterior.definition.v2"
        target["stateVersion"] = str(target.get("stateVersion") or target.get("state_version") or "game.motherShipInterior.state.v1")

        scalar_default("enabled", True)
        scalar_default("initialLocation", defaults.get("initialLocation", "bay.shuttle"))
        scalar_default("initialObjective", defaults.get("initialObjective", "objective.bay-ops"))
        scalar_default("power", defaults.get("power", "emergency"))
        scalar_default("security", defaults.get("security", "quarantine"))

        for key in ("locations", "objectives", "doors", "terminals", "flags", "spawns", "movement", "interactions", "validation"):
            object_default(key)
        for key in ("rooms", "exits", "props", "interactables"):
            list_default(key)

        target.setdefault("validation", {})
        if isinstance(target["validation"], dict):
            target["validation"].setdefault("requireDefinitionVersion", True)

        migrations = []
        if source_version != target["definitionVersion"]:
            migrations.append(f"{source_version}->{target['definitionVersion']}")

        target["migration"] = {
            "schema": "game.motherShipInterior.migration.v1",
            "sourceDefinitionVersion": source_version,
            "targetDefinitionVersion": target["definitionVersion"],
            "stateVersion": target["stateVersion"],
            "migratedAtLoad": bool(migrations or defaults_applied),
            "migrations": migrations,
            "defaultsApplied": sorted(set(defaults_applied)),
        }
        return target

    def _game_project_apply_runtime_migrations(self, project: dict[str, Any]) -> dict[str, Any]:
        if not isinstance(project, dict):
            return project
        result = json.loads(json.dumps(project))
        fallback = self._game_mother_ship_default_interior()
        for scene in result.get("scenes", []):
            ...
            if not isinstance(shuttle, dict):
                continue
            interior = shuttle.get("motherShipInterior")
            if isinstance(interior, dict):
                shuttle["motherShipInterior"] = self._game_migrate_mother_ship_interior_for_read(interior, fallback)
        return result
    ...
    def _game_gameplay_pack_js_source_payload(self, body: dict[str, Any]) -> dict[str, Any]:
        ...
        if requested in {"", "None", "none", "no-pack", "no_pack", "base"}:
            return {
                "ok": True,
                "schema": "game.gameplayPackJsSourceResult.v1",
                "kind": "gameplay-pack-js-source-result",
                "mode": "none",
                "project_id": root.name,
                "requestedPackId": requested or "None",
                "loaded": False,
                "source": "",
                "runtimeLoaded": False,
                "projectJsonModified": False,
