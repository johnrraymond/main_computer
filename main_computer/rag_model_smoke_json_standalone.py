...
from main_computer.providers import LLMProvider, get_provider
from main_computer.rag_harness import RagRunResult, run_rag_harness
...
def _log_executor_result(result: ExecutorToolLoopResult) -> None:
    ...
    for step in _summarize_executor_steps([item.to_dict() for item in result.steps]):
...
def _json_safe_executor_result(executor_result: ExecutorToolLoopResult | None) -> dict[str, Any] | None:
    return executor_result.to_dict() if executor_result else None
...
def validate_csv_audit_model_smoke(result: RagRunResult, *, strict: bool) -> RagModelSmokeReport:
    ...
    retrieved = set(result.retrieved_paths)
    ...
    found_required = sorted(path for path in required_context_any if path in retrieved)
    if len(found_required) < 2:
        failures.append(
            "Expected RAG retrieval to include at least two executor-related files; "
            f"found {found_required or 'none'}."
        )
    ...
    if not evidence:
        warnings.append("Expected final_plan.evidence to cite retrieved files.")
    else:
        uncited = []
...
def validate_broken_code_repair_docker_smoke(
    ...
    *,
    ...
    found_context = sorted(path for path in expected_context if path in retrieved)
    if len(found_context) < 1:
        warnings.append(
            "Expected Docker executor and executor tool-loop context in retrieval; "
...
def run_csv_audit_model_smoke(
    *,
    ...
    if verbose:
        print(f"[rag-model-smoke] using {_provider_summary(provider)}")
        print(
    ...
    if verbose:
        _log_rag_result("csv-audit", result)
        if dump_json:
            print("[rag-model-smoke] raw csv-audit RAG run:")
            print(_json_dumps(result.to_dict()))

    report = validate_csv_audit_model_smoke(result, strict=strict)

    if verbose:
        print("[rag-model-smoke] csv audit validation report:")
...
def _broken_code_repair_executor_context(result: RagRunResult) -> str:
    ...
    lines = [
        ...
        "",
        "Retrieved repository context paths:",
    ]
    for path in result.retrieved_paths[:24]:
        lines.append(f"- {path}")
    if len(result.retrieved_paths) > 24:
        lines.append(f"- ... {len(result.retrieved_paths) - 24} more paths omitted")
    if final_plan:
        lines.extend(
            [
                "",
...
def run_broken_code_repair_docker_smoke(
    *,
    ...
    docker_executor = DockerExecutor(enabled=True)
