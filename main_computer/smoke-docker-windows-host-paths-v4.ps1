# smoke-docker-windows-host-paths-v4.ps1
#
# Diagnostic + smoke test for Docker-on-Windows host-drive display behavior.
#
# This version adds:
# - CLI/config preflight checks
# - a one-shot Docker config probe before starting the server
# - a running-container config probe
# - an internal-container API probe
# - default host port 8875 to avoid accidentally hitting a local dev server on 8765
#
# Run from the repository root:
#   powershell -ExecutionPolicy Bypass -File .\smoke-docker-windows-host-paths-v4.ps1
#
# Optional:
#   powershell -ExecutionPolicy Bypass -File .\smoke-docker-windows-host-paths-v4.ps1 -Port 8765 -KeepContainer
...
try {
    ...
    $runningProbeRaw = & docker exec $ContainerName python -c $ConfigProbe
    if ($LASTEXITCODE -ne 0) {
    ...
    $InternalApiProbe = 'import urllib.request; print(urllib.request.urlopen("http://127.0.0.1:8765/api/path-mounts", timeout=10).read().decode())'
    $internalRaw = & docker exec $ContainerName python -c $InternalApiProbe
    if ($LASTEXITCODE -ne 0) {
