#!/usr/bin/env python3
"""Contract-driven assembly for the existing AI-backed RAG model smoke.

This is a new versioned assembly path. It does not replace or edit
``main_computer.rag_model_smoke``. The selected target smoke remains the
CSV-audit scenario from that module; this runner only breaks the smoke into
smaller boundaries with explicit state/evidence contracts.

Default local-AI run:

    python -m main_computer.rag_model_smoke_assembly_v1 --repo-dir . --output-dir diagnostics_output/rag_model_smoke_assembly_v1

Deterministic contract-test run, useful for pytest and CI structure checks:

    python -m main_computer.rag_model_smoke_assembly_v1 --provider-mode fake --json
"""
...
ASSEMBLY_VERSION = "rag_model_smoke_assembly_v1"

SCRIPT_MODULES: tuple[str, ...] = (
    "main_computer.rag_model_smoke_subscripts_v1.target_inventory",
    "main_computer.rag_model_smoke_subscripts_v1.provider_gate",
    "main_computer.rag_model_smoke_subscripts_v1.csv_audit_model_run",
    "main_computer.rag_model_smoke_subscripts_v1.report_contract",
)
...
def load_contract(module: str, repo_dir: Path | str = ".") -> dict[str, Any]:
...
def _validate_result(contract: dict[str, Any], result: dict[str, Any], state_before: dict[str, Any]) -> list[str]:
    ...
    missing_state = [key for key in _contract_keys(contract, "requires") if key not in state_before]
    if missing_state:
...
def run_assembly(
    *,
    ...
    provider_mode = str(provider_mode or "local").strip().lower()
    if provider_mode not in {"local", "fake"}:
        raise ValueError("provider_mode must be 'local' or 'fake'")
    ...
    state: dict[str, Any] = {
        ...
        "assembly_run_id": f"{ASSEMBLY_VERSION}_{_utc_run_id()}",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        choices=["local", "fake"],
        default="local",
        help="local uses the existing Ollama-backed smoke; fake is deterministic contract-test mode.",
    )
    ...
    result = run_assembly(repo_dir=args.repo_dir, output_dir=args.output_dir, provider_mode=args.provider_mode)
