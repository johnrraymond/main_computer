...
def _expand_includes(text: str, base_dir: Path, seen: tuple[Path, ...] = ()) -> str:
    def replace(match: re.Match[str]) -> str:
        ...
        if include_name == "applications/scripts/mcel-application-package-catalog.js":
            from main_computer.mcel_application_virtual_assets import read_virtual_mcel_browser_asset

            return read_virtual_mcel_browser_asset(_REPOSITORY_ROOT, include_name).decode("utf-8")

        include_path = (base_dir / include_name).resolve()
        web_root = _WEB_DIR.resolve()
        if not (web_root == include_path or web_root in include_path.parents):
            raise ValueError(f"Viewport include escapes the web asset directory: {include_name}")
