...
selected_symbol = trace.get("selected_symbol", {}).get("selected_symbol")
...
def run_smoke(*, repo_dir: Path, model: str | None, stream_model: bool) -> SmokeTrace:
    ...
    evidence_grader = call_evidence_grader(provider, query, merged_hits)
    ...
    selected_symbol = call_symbol_disambiguator(provider, query, symbols, evidence_grader)
    final_answer = call_final_answer(provider, query, included_hits, selected_symbol)
