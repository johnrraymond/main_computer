...
def main() -> int:
    ...
    try:
        ...
        try:
            ensure_sqlite_publish_smoke_source(smoke_repo, abs_runtime_site)
        except Exception as exc:
            message = str(exc)
            accepted_messages = ("must be site-relative", "escaped the website directory")
            if any(part in message for part in accepted_messages):
                smoke.pass_(
                    "absolute /app/data path is not accepted as current source path",
                    f"Raised expected {type(exc).__name__}: {message}",
                )
            else:
                smoke.fail(
                    "absolute /app/data path is not accepted as current source path",
                    f"Raised {type(exc).__name__}, but message did not identify site-relative/source-path rejection: {message}",
                )
        else:
            smoke.fail(
                "absolute /app/data path is not accepted as current source path",
