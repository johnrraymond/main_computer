...
def _validate_web_search_smoke(
    *,
    ...
    # missing Tor is reported as a warning so this smoke can still validate the
    # AI -> router -> Tor-search wiring on machines without a live Tor service.
...
def run_rag_web_search_smoke(
    *,
    ...
    reached the Tor-only DuckDuckGo connector. With ``strict=True`` it also
    requires live DuckDuckGo-onion results through a configured Tor proxy.
    """
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--strict", action="store_true", help="Require live Tor DuckDuckGo results and treat warnings as failures.")
