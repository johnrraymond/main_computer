#!/usr/bin/env python3
"""Verbose RAG-docket prompt quality smoke tester v3.
...
Why this version exists:
  ...
  * It streams what the app knows while the POST is still running by polling:
    ...
    and reports backend heartbeat events at a throttled cadence.
  * It polls `ollama ps` during the run so you can see whether Ollama has actually
    loaded or is running the requested model.
  * It prints raw request payloads, raw HTTP error bodies, route result changes,
    diagnostics locations, and a deterministic operator-side score.
...
def is_backend_heartbeat(event: dict[str, Any]) -> bool:
    """Return True for the noisy repeated "still waiting" model-stream heartbeat."""
    data = event.get("data") if isinstance(event.get("data"), dict) else {}
    title = str(event.get("title") or "").lower()
    message = str(event.get("message") or data.get("running_text") or data.get("history_label") or "").lower()
    rag_type = str(data.get("rag_type") or "").lower()
    status = str(event.get("status") or "").lower()
    content_chars = data.get("content_chars")
    thinking_chars = data.get("thinking_chars")
    return (
        status == "running"
        and rag_type == "model_stream"
        and (
            "model request still waiting" in title
            or "request is still waiting" in message
            or "still waiting for the http response" in message
        )
        and str(content_chars or "0") == "0"
        and str(thinking_chars or "0") == "0"
    )


def heartbeat_summary(event: dict[str, Any]) -> dict[str, Any]:
    data = event.get("data") if isinstance(event.get("data"), dict) else {}
    return {
        "ts": event.get("ts") or event.get("time") or utc_now(),
        "source": event.get("source"),
        "status": event.get("status"),
        "rag_type": data.get("rag_type"),
        ...
        "content_chars": data.get("content_chars"),
        "thinking_chars": data.get("thinking_chars"),
        "message": compact(event.get("message") or data.get("running_text") or data.get("history_label") or "", limit=350),
        "log_file": data.get("log_file"),
    }


def run_ollama_ps(args: argparse.Namespace) -> dict[str, Any]:
    command = [args.ollama_command, "ps"]
    ...
    try:
        completed = subprocess.run(
            ...
            timeout=max(1.0, float(args.ollama_ps_timeout_s)),
            check=False,
        )
        return {
        ...
        return {
            ...
            "elapsed_s": round(time.monotonic() - started, 3),
            "stdout": "",
            "stderr": f"command not found: {args.ollama_command!r}",
            "ts": utc_now(),
        }
        ...
        return {
            ...
            "stderr": f"timeout after {args.ollama_ps_timeout_s}s",
            "ts": utc_now(),
        }
    except Exception as exc:
        return {
...
class OllamaPsState:
    ...
    def poll(self, args: argparse.Namespace, *, force: bool = False, reason: str = "interval") -> dict[str, Any] | None:
        if getattr(args, "no_ollama_ps", False):
            return None
        now = time.monotonic()
        interval = max(0.1, float(args.ollama_ps_interval_s))
        if not force and self.records and now - self.last_run_at < interval:
            return None

        result = run_ollama_ps(args)
        self.last_run_at = now
        self.records.append(result)

        signature = stable_json(
            {
                "ok": result.get("ok"),
        ...
        if force or changed or getattr(args, "ollama_ps_always_print", False):
            ...
            display = {
                ...
                "stdout": stdout if stdout else "(no running Ollama models reported by `ollama ps`)",
                "stderr": stderr,
                "ts": result.get("ts"),
            }
            print_block("ollama ps", display, limit=args.json_limit)
        return result
...
def stream_until_done(
    ...
    *,
    ...
    ollama_state = OllamaPsState()
    done_seen_at: float | None = None
    stream_started = time.monotonic()
    heartbeat_count = 0
    heartbeat_suppressed_since_print = 0
    heartbeat_every = max(1, int(args.heartbeat_every))

    print(f"\n[stream] started for run_id={run_id}", flush=True)
    print(f"[stream] polling {RUN_RESULT_ROUTE}, {ACTIVITY_EVENTS_ROUTE}?filter=ai, {ACTIVITY_EVENTS_ROUTE}?filter=live, and `{args.ollama_command} ps`", flush=True)
    print(f"[stream] heartbeat throttle: showing every {heartbeat_every} backend waiting heartbeat(s); non-heartbeat events still print immediately.", flush=True)
    if not args.tail_log:
        print("[stream] backend session.log tail is suppressed by default; use --tail-log to print it.", flush=True)

    ollama_state.poll(args, force=True, reason="stream_start")

    while True:
        now = time.monotonic()
        post_done = worker.finished_at is not None

        # `ollama ps` is intentionally independent of backend events. If Ollama
        # is not actually loading/running a model, this shows that directly.
        ollama_state.poll(args, force=False, reason="interval")
        ...
        for payload in (activity_ai, activity_live):
            for event in relevant_events([payload], run_id):
                ...
                if is_backend_heartbeat(event):
                    heartbeat_count += 1
                    heartbeat_suppressed_since_print += 1
                    if heartbeat_count % heartbeat_every == 0:
                        suppressed_before_this = max(0, heartbeat_suppressed_since_print - 1)
                        print(
                            f"\n[heartbeat #{heartbeat_count}] showing this backend waiting heartbeat; "
                            f"suppressed {suppressed_before_this} similar heartbeat(s) since the last heartbeat print.",
                            flush=True,
                        )
                        print_block("heartbeat summary", heartbeat_summary(event), limit=args.json_limit)
                        if args.full_heartbeat_json:
                            print_block("full heartbeat event JSON", event, limit=args.json_limit)
                        ollama_state.poll(args, force=True, reason=f"heartbeat_{heartbeat_count}")
                        heartbeat_suppressed_since_print = 0
                    continue

                print_event(event, full_json=args.full_event_json, json_limit=args.json_limit)
                # Non-heartbeat activity is a useful moment to correlate the app
                # docket with the real Ollama process table.
                ollama_state.poll(args, force=True, reason="non_heartbeat_activity")
        ...
        if post_done:
            if done_seen_at is None:
                ...
                print("\n[stream] POST completed; draining activity/ollama state briefly.", flush=True)
                ollama_state.poll(args, force=True, reason="post_completed")
        ...
        if now - stream_started > args.stream_timeout_s:
            ...
            ollama_state.poll(args, force=True, reason="stream_timeout")
            break

        time.sleep(args.stream_interval_s)

    if heartbeat_suppressed_since_print:
        print(f"\n[heartbeat] suppressed {heartbeat_suppressed_since_print} backend waiting heartbeat(s) after the last printed heartbeat.", flush=True)
    ...
    if args.tail_log:
        ...
        print("\n[stream] log tail suppressed. Backend log file(s):", flush=True)
        for log_file in log_files:
            print(f"  {log_file}", flush=True)

    ollama_state.poll(args, force=True, reason="stream_end")
    ...
    write_json_file(output_dir / "stream" / f"{run_id}_ollama_ps.json", ollama_state.records)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--heartbeat-every", type=int, default=10, help="Print only every Nth repeated backend waiting heartbeat. Default: 10.")
    parser.add_argument("--tail-log", action="store_true", help="Print backend session.log tail chunks. Default is off to avoid backend heartbeat spam.")
    parser.add_argument("--ollama-command", default=os.environ.get("OLLAMA_COMMAND", "ollama"), help="Command used for `ollama ps`.")
    parser.add_argument("--ollama-ps-interval-s", type=float, default=5.0, help="Minimum interval between background `ollama ps` polls.")
    parser.add_argument("--ollama-ps-timeout-s", type=float, default=5.0, help="Timeout for each `ollama ps` call.")
    parser.add_argument("--ollama-ps-always-print", action="store_true", help="Print every `ollama ps` poll instead of only changes/forced checkpoints.")
    parser.add_argument("--no-ollama-ps", action="store_true", help="Disable `ollama ps` polling.")
    ...
    parser.add_argument("--full-heartbeat-json", action="store_true", help="Also print full JSON for the every-Nth heartbeat.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    suite_meta = {
        "schema": "rag_docket_prompt_tester.verbose.v3",
        "run_id": args.run_id,
        ...
        "tail_log": bool(args.tail_log),
        "ollama_ps": not bool(args.no_ollama_ps),
        "ollama_command": args.ollama_command,
        "ollama_ps_interval_s": float(args.ollama_ps_interval_s),
        "note": "Per-case run IDs are unique by default to avoid FileExistsError/HTTP 400 from existing diagnostics dirs. v3 also polls `ollama ps` and throttles repeated backend waiting heartbeats.",
    }
