...
Use `pretty_docs/mcel-status-and-roadmap.md` for current maturity, generated-baseline interpretation, and authorized future work. This guide explains the system; it is not a patch ledger and must not override registry or repository-audit output.
...
The high-level problem that motivates the next AI-facing authoring layer is documented by concrete code examples in `pretty_docs/mcel-ai-authoring-language-executive-overview.md`. The governing semantic boundary is specified in `pretty_docs/mcel-ai-authoring-semantic-boundary.md`: one official vanilla-JavaScript syntax, one declaration per independent semantic decision, generated mechanical repetition, and proof that accounts for every consequential side effect. `pretty_docs/mcel-application-ir-and-compiler-migration.md` then defines the stable compiler architecture: documentation/requirements front ends, current explicit or normalized application forms, and the future DSL converge on one source-independent MCEL Application IR; generated projections remain replaceable, and runtime evidence remains independent. `pretty_docs/mcel-application-ir-schema-and-normalization.md` defines the proposed IR node families, constrained semantic records, normalization, provenance, fingerprints, and acid-app slices. `pretty_docs/mcel-existing-application-definition-migration-inventory.md` identifies every current definition family and the preservation path for Git Tools, Code Editor, Document Editor, and the remaining requirements, blueprint, scaffolded, and legacy applications. `pretty_docs/mcel-constrained-expression-model.md` specifies the pure typed expression vocabulary, static analysis, domain-operator extension rule, legacy opaque-function quarantine, and strict boundary between calculated meaning and capability effects. `pretty_docs/mcel-consequential-effects-and-proof-accounting.md` specifies how declared effects, runtime effect instances, receipts, SCM/capability/browser evidence, terminal dispositions, cleanup, uncertainty, recovery, and residue reconcile before proof may pass. `pretty_docs/mcel-official-vanilla-javascript-dsl.md` fixes the proposed source notation that constructs those semantics: one strict CommonJS vanilla-JavaScript form, stable handles and IDs, constrained callbacks, app-local modules, semantic surfaces/layouts, capability lifecycles, and ordered proof scenarios. `pretty_docs/mcel-compiler-diagnostics-and-repair-protocol.md` defines how every front end and canonical stage reports stable semantic failures, safe repair choices, invalidated evidence, and the exact stage where an AI resumes. `pretty_docs/mcel-scaffolder-generated-projection-and-compatibility.md` defines the concrete workspace and ownership boundary: live versus candidate packages, authored/generated/legacy/evidence classes, DSL and explicit scaffold modes, legacy importers, IR back ends, package and browser projections, compatibility reports, promotion, rollback, and generated-file drift. `pretty_docs/mcel-ai-application-authoring-cycle.md` defines the exact stage gates and re-entry rules an AI follows, while `pretty_docs/mcel-ai-authoring-pattern-catalog.md` provides complete example-first patterns that connect user requests to DSL declarations, generated mechanics, diagnostics, migration, and proof. `pretty_docs/mcel-semantic-change-and-evidence-impact.md` defines how later edits are compared through semantic dependency graphs, how evidence is invalidated or reused, and how the AI returns to the earliest affected stage. `pretty_docs/mcel-ai-authoring-and-migration-benchmark.md` defines the controlled creation, migration, repair, modification, effect, evidence, and proof trials used to decide whether the proposed DSL is actually better for AI application development. `pretty_docs/mcel-ai-authoring-documentation-completeness-review.md` audits the complete set, closes the initial v1 ambiguities, and limits the first permissible implementation to the structural IR kernel after explicit authorization. Read this set before evaluating a compiler, migration, scaffolder, or DSL implementation.

The current application-authoring model across HTML, application contract data, JavaScript behavior, layout hints, user layout operations, themes, chrome, and FLOG is documented in `pretty_docs/mcel-application-authoring.md`.

The requirements-language model for documentation-first app contracts is documented in `pretty_docs/mcel-requirements-language.md`. It defines the shared `mcel-*` grammar, status vocabulary, risk vocabulary, responsibility-based layout vocabulary, adapter truth-gate vocabulary, parser expectations, and the registry commands `python tools/mcel_requirements_registry.py`, `--report`, and `--lab-json` used by app requirements docs, MCEL Lab, and repository truth auditing.

The Code Editor requirements contract is documented in `pretty_docs/mcel-code-editor-requirements.md`. Its registered adapter reports full application semantic readiness for a bounded source-safe authoring scope; command execution remains a separate prohibited or advanced boundary.

The Git Tools requirements contract is documented in `pretty_docs/mcel-git-tools-requirements.md`. Its registered adapter reports full application semantic readiness for the governed preflight-oriented scope while evidence-only preparation remains distinct from Git or remote mutation.

The Calculator requirements contract is documented in `pretty_docs/mcel-calculator-requirements.md`. Its registered adapter covers deterministic compute, graphing, explicit Mathics/model lanes, evidence, and recovery without authorizing hidden file, Git, shell, package, or publish mutation.

The File Explorer requirements contract is documented in `pretty_docs/mcel-file-explorer-requirements.md`. Its registered adapter covers the bounded read-only roots, navigation, listing, search, preview, classification, evidence, and recovery scope; cross-app handoff remains separately governed.

The Website Builder requirements contract is documented in `pretty_docs/mcel-website-builder-requirements.md`. Its registered adapter covers bounded site authoring, runtime setup, publish-lane preflight, evidence, and Git Tools handoff preparation.
...
That specification works forward from the ideal MCEL 1.0 application while identifying the bridges required from requirements, adapter, SCM, surface, layout, observation, acceptance, and truth authorities. Its deterministic repository-local generator, structural validator, golden fixture, package catalogs, browser-safe runtime projection, SCM-controlled application runtime, generic semantic surface mount, package-local acceptance, operation-linked Chromium observation, and app-oriented proof orchestration are live. It does not supersede the repository-aware executable authoring path in `pretty_docs/mcel-application-authoring.md`. Current code authorization remains exclusively in `pretty_docs/mcel-status-and-roadmap.md`.
...
| Question | Authority |
| --- | --- |
| What product behavior is specified? | Requirements registry and app requirements docs |
| Is a semantic adapter registered and complete? | Domain adapter registry |
| Is an app enrolled and what maturity is declared? | App surface registry |
| Does runtime surface evidence pass? | Repository-bound FLOG evidence evaluated by surface conformance and the truth gate |
| Does acceptance evidence pass? | Acceptance runner, binding catalog, and truth gate |
| What is the aggregate repository state? | Repository truth audit |
| What may an observation claim? | Epistemic status and observation-bundle contracts |

A declared `semantic-runtime` level or `fullApplicationSemanticReady` adapter snapshot is not the same as repository-bound runtime or acceptance proof.
...
| Browser proof | `mcel-browser-observer.js`, `mcel-browser-runner.js` | Runs the existing browser-proof pathway; it does not yet produce the newer repository-bound observation-bundle contract. |
