...
AI-1 through AI-9.4.1 are implemented, and this snapshot now includes the constrained AI-10A character foundation: the strategic coordinator preserves its existing cognition and authority boundaries; Vela, Solace, travel, and the non-obscuring strategic dock remain intact; and the WebGL game now has one stable raider boarder, Engineering Officer Mara Venn, and a stable raider vessel identity behind bounded perceptions, legal actions, receipts, local persistence, nonblocking remote-policy support, and deterministic fallback. Repository-backed strategic and character saves, a real model call, broader scene performance, and learned policies remain unimplemented.
